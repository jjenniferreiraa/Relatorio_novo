...
A perfuração de um poço de petróleo ocorre em diferentes etapas, dentre elas, o início de poço, que ocorre com o assentamento do revestimento condutor. Também chamando de revestimento estrutural, ele é responsável por dar sustentação às paredes do poço e por servir de suporte para os equipamentos de cabeça de poço e os revestimentos das fases seguintes da perfuração. O condutor é instalado no solo e o seu processo de instalação vai depender de variáveis como localização do poço – \textit{onshore} ou \textit{offshore} – e do tipo de solo.  

A cravação é um dos métodos para iniciar o poço em ambiente \textit{offshore}. A depender da profundidade da lâmina d’agua, o ambiente pode ser hostil para a execução de estudos experimentais e, consequentemente, tornar o projeto de poço mais demorado e oneroso. Uma alternativa válida para diminuir os custos de projeto é realizar estudos numéricos a partir de um modelo computacional devidamente validado e calibrado, capaz de representar fielmente a real instalação de um revestimento condutor no solo.  

Este apêndice, portanto, tem como objetivo apresentar os estudos numéricos sobre a operação de instalação do revestimento condutor por cravação. A modelagem foi desenvolvida com base em dados da instalação de condutores no campo Papa Terra, provenientes do relatório de instalação por martelamento e de relatórios de sondagem com CPTu realizados neste campo. 

Apurou-se das aplicações realizadas anteriormente que modelar a cravação do condutor através de métodos numéricos fundamentados na mecânica do contínuo é inviável devido à excessiva distorção de malha, causada pelas grandes deformações inerentes ao processo.  Uma alternativa para superar este desafio, e que vem ganhando força na comunidade geotécnica, é utilizar métodos numéricos baseados em partículas, tais como o Método dos Pontos Materiais (MPM). Este método tem se destacado em cenários com grandes deformações, e é uma escolha promissora para o desenvolvimento da modelagem em questão.

\section{Modelagem numérica}

O estudo numérico foi desenvolvido no ANURA3D, \textit{software} de código aberto e baseado no MPM. Para que um modelo numérico seja devidamente construído, é necessário definir os modelos físicos e matemáticos adequados para representar o comportamento do sistema estudado. A seguir, uma breve descrição das formulações e dos algoritmos utilizados na modelagem numérica do problema proposto, assim como a descrição de sua geometria, restrições e propriedades dos materiais.

\subsection{Método dos Pontos Materiais}

O MPM, originalmente desenvolvido por \citet{sulsky1994particle}, pertence à família de métodos baseados em partículas, os quais têm sido implementados para superar problemas com distorção excessiva de malha dos métodos numéricos tradicionais. Este método deriva do método particle-in-cell (PIC), utilizado na mecânica dos fluidos \citet{ceccato2017numerical}. A Figura abaixo mostra uma básica ilustração do algoritmo do MPM.

\begin{figure}[H]
	\centering
	\includegraphics[width=0.8\linewidth]{cravacao/MPM}
	\caption{Algoritmo de resolução do MPM. a) Configuração inicial; b) transferência das informações de propriedades do material para os nós; c) resolução das equações de equilíbrio nos nós da malha computacional (fase lagrangiana); d) informações são transferidas dos nós para os pontos materiais, os quais têm sua posição atualizada e malha retorna ao estado original, descartando os dados (fase convectiva); e) configuração deformada (atualizada). Fonte: Adaptado de \citet{zambrano2020numerical}.}
	\label{fig:mpm}
\end{figure}

O MPM possui abordagem euleriana-lagrangiana, na qual o espaço é discretizado em uma malha euleriana, enquanto o corpo contínuo modelado é discretizado em uma quantidade finita de pontos materiais que guardam as propriedades do material analisado. Essas partículas transferem os valores das variáveis de estado do material para os nós da malha, sobre os quais se avaliam as equações de movimento. A atualização dos cálculos nos nós é transferida aos pontos materiais e, ao final de cada passo de tempo, a malha é reestruturada para a posição inicial e as partículas permanecem na posição deslocada, prevenindo distorções excessivas nos elementos (\citet{martinelli2020numerical}, \citet{yost2022}, \citet{fu2024material}).

\subsection{\textit{Two-phase single point formulation:} modelagem para problemas multifásicos}

No MPM, uma formulação dinâmica explícita foi desenvolvida para modelar a interação solo-água. Nela, considera-se que a geração e a dissipação de pressão de poros da água, durante a instalação da estrutura, levando em conta ondas dinâmicas tanto na fase da água quanto na fase sólida (\citet{ceccato2017numerical}; \citet{galavi2019}). As equações de governo são conservação de massa, conservação de momento e um modelo constitutivo capaz de prever a solução do sistema, descritas as equações 1 a 5:
